...
const CASE_STUDIES = {
  collaborative: {
    ...
    researchNarrative:
      "I led synthesis of the usability findings and brought them to leadership across product, sales, and customer success. This wasn't just a design problem — it was a revenue and retention risk. Getting buy-in required connecting the UX failures directly to churn and competitive displacement. We got the green light to start over.",
    process: [
      {
        phase: "Audit",
      ...
      },
      {
        ...
        body: "Structured sessions across 14 inspection companies surfacing failure points, mental models, and the three root causes: people couldn't find key information, the fulfillment path was unclear, and file uploads were error-prone.",
      },
      {
    ...
    screens: [
      ...
      },
      {
        ...
        caption: "Vendors upload their inspection PDF and the system extracts answers into the structured form — turning the extra effort of double-entry into a quick review pass.",
      },
      {
    ...
    pull:
      "Task success went from under 50% — and as low as 0% — to 94%. Every single participant said the new design was an improvement.",
  },
  ...
  compliance: {
    ...
    context:
      "Before a subcontractor can work on a construction project, they need to provide proof of insurance — specific documents, in required coverage amounts, current. For most teams that process lived entirely in email and cloud storage. Compliance managers were chasing documents and cross-referencing coverage amounts by hand across multi-year projects.",
    contextLong:
      "The stakes ran two directions. Payments couldn't go out to non-compliant vendors — creating bottlenecks that slowed projects and frustrated subs. And if a project was audited, compliance managers had to prove every vendor was covered throughout the entire project life. One missing document was real legal and financial exposure. The opportunity: make compliance trackable and automated, eliminate manual chase work, and give teams confidence their projects were covered at every stage.",
    research: [],
    researchNarrative:
      "I led design from discovery through delivery, starting with journey mapping to build shared understanding across stakeholders. Mapping the existing workflow made the fragmentation visible in a way that aligned the team quickly — and made the case for why a purpose-built solution mattered. Wireframes for internal validation came first, then higher fidelity for customer testing.",
    process: [
      {
        phase: "Journey map",
      ...
      },
      {
        ...
        body: "Compliance status had to surface inside the payments flow, and documents had to be requestable before a payment could go out. Required tight collaboration with the payments team to design one experience, not two products that happen to touch.",
      },
    ],
    screens: [
      { label: "Compliance templates", aspect: "16/10", src: "images/compliance-screen-01.png", caption: "Compliance managers define the requirements for a project once — coverage types, minimum amounts, expirations — and the template applies to every vendor on the job." },
      { label: "Vendor document request — reply-based upload", aspect: "16/10", caption: "Vendors get a request link and upload directly, no account creation or training required. Replies route back into the project automatically." },
      { label: "Automated extraction · mapped to requirements", aspect: "16/10", src: "images/compliance-screen-03.png", caption: "The platform reads uploaded certificates, extracts coverage data, and maps it against requirements — flagging gaps before a payment goes out." },
  ...
  genui: {
    ...
    context:
      "When we launched our AI assistant, it could only respond in text. That worked for simple queries, but the platform handles complex financial data — draws, inspections, compliance status, payment timelines — and a text response often isn't the right container for that information.",
    contextLong:
      "The opportunity had two parts. The design system was missing components needed to support the new AI features. More ambitiously, I wanted to go beyond filling gaps and design GenUI organisms that could dynamically render based on what a user actually asked for. Ask about a payment and get a structured card. Ask about inspection data and get a chart. The response shapes itself to the question. This is the first version of that base set.",
    research: [
    ...
    researchNarrative:
      "Before designing anything, I needed to understand what users were actually looking for when they turned to AI. I ran two audits in parallel: an analysis of the most frequently visited pages in the product, and a review of how users were interacting with the existing text-only AI. The data — not a guess — set the priority order for the component set.",
    researchSkim:
      "Two parallel audits — most-visited pages + existing AI interactions — set the priority order for the component set. Data drove the roadmap, not guesses.",
    process: [
      {
        phase: "Audit",
      ...
      },
      {
        ...
        body: "Designed cards, tables, charts, and the agent response shell itself — the container and interaction pattern for how AI surfaces any of the above in context.",
      },
      {
        phase: "Build with AI",
        title: "Figma + Claude",
      ...
      },
      {
        ...
        body: "Reviewed with the lender and data teams. Both said the set covers more than what they need — a meaningful bar given the complexity they work with daily.",
      },
    ],
    screens: [
      { label: "Callout cards · tone-driven, single-record summaries", aspect: "842/1082", frame: "panel", fit: "contain", src: "images/genui-screen-01.png", caption: "A structured snapshot when the answer is a single record — payment, draw, inspection. Tone (negative, attention, positive, neutral) drives accent, icon, and the recommended action." },
      { label: "Prompt suggestions · lists & follow-up chips", aspect: "852/640", frame: "panel", fit: "contain", src: "images/genui-screen-02.png", caption: "When the agent doesn't yet have a question, prompt suggestions kick things off — surfaced from common product flows and the user's recent activity." },
      { label: "Charts · financial & inspection data", aspect: "856/1448", frame: "panel", fit: "contain", src: "images/genui-screen-03.png", caption: "For trend, distribution, and forecast questions, the agent reaches for a chart — drawing on the same financial data that lives elsewhere in the platform." },
      ...
      { label: "Agent response shell · wraps any primitive", aspect: "834/948", frame: "panel", fit: "contain", src: "images/genui-screen-05.png", caption: "The shared container any GenUI primitive renders inside. One consistent interaction pattern — thought process, flagged signals, follow-ups — across cards, tables, and charts." },
    ],
    pull:
  ...
  money: {
    ...
    context:
      "Not every design problem requires starting from scratch. Payment Improvements was a pair of focused UX improvements driven directly by customer feedback — each one targeting a specific point of friction in the payments experience.",
    contextLong:
      "The goal across both was the same: reduce cognitive load, cut unnecessary steps, and make the software feel like it's working for the user instead of the other way around. Two different problems, one consistent approach — find where the interface is asking users to do work the software could do instead, and fix it.",
    ...
    improvements: [
      {
        ...
        oneLiner: "One status, one action — with the rest available on demand.",
        problem:
          "The original tracker surfaced too much at once. Multiple statuses, multiple contextual actions, and the path forward fighting with the path behind for attention. Users had to parse the whole component just to find the next thing they could do.",
        approach:
          "Streamline to the single relevant action for the current status. Previous and upcoming steps are still there — tucked behind a track button — but they no longer compete with the one decision the user actually needs to make right now.",
      ...
      },
      {
        ...
        problem:
          "The Send Payment flow was a four-step wizard — amount, account, review, send — each step on its own full screen. For a task that happens dozens of times a day, every extra click and page load compounded into real friction.",
        approach:
        ...
        outcome:
          "Two clicks instead of four, dozens of times a day — compounded over a year, the time saved is real.",
      },
    ],
    pull:
      "Two clicks instead of four, dozens of times a day — the time saved compounds.",
  },
};
